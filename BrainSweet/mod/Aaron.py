...
class HeatMap:
    ...
    def generateHeatmap(self):
        if not (self.POS_FLAG and self.SCORE_FLAG):
            ...
            self.Heatmap = convolutionHeatmap(self.timestep, self.row, self.col, self.depth, self.Score, self.PosX,
                                              self.PosY, self.PosZ, self.brainDMask)
    ...
    def show(self):
        if not self.HEATMAP_FLAG:
            ...
            my_model = MyModel(self.brainMask, self.Heatmap, self.PosX, self.PosY, self.PosZ, self.HeatmapVmin,
                               self.HeatmapVmax, self.timestep)
...
class MyModel(HasTraits):
    ...
    def update_time(self):
        ...
        if self.plot is None:
            ...
            self.scene.mlab.pipeline.iso_surface(background, vmin=0, vmax=1, opacity=1.0, colormap='gray',
                                                 figure=self.scene.mayavi_scene)

            heat = self.scene.mlab.pipeline.scalar_field(heatmap)
            self.plot = self.scene.mlab.pipeline.volume(heat, vmax=self.vmin + .8 * (self.vmax - self.vmin),
                                                        vmin=self.vmin, figure=self.scene.mayavi_scene)
    ...
    def update_points(self):
        ...
        if self.electrode == ELECTRODE_OFF:
            ...
            self.elec = self.scene.mlab.points3d(electronX, electronY, electronZ, scale_factor=10, resolution=20,
                                                 scale_mode='none', color=(1, 0, 1), opacity=1.0,
                                                 figure=self.scene.mayavi_scene)
            #############################################################################
